...
function Log(eventBus, notifications, tokenSimulationPalette, canvas) {
  ...
  eventBus.on(GENERATE_TOKEN_EVENT, function(context) {
    ...
    if (is(element, 'bpmn:StartEvent')) {
      self.log(elementName || 'Iniciar Simulação', 'info', 'bpmn-icon-start-event-none');
    } else if (is(element, 'bpmn:Task')) {
      self.log(elementName || 'Tarefa', 'info', 'bpmn-icon-task');
	  	  //alert('ssss'+elementName);
    } else if (is(element, 'bpmn:BusinessRuleTask')) {
      self.log(elementName || 'Tarefa de Regra de Negócios', 'info', 'bpmn-icon-business-rule');
    } else if (is(element, 'bpmn:ManualTask')) {
      self.log(elementName || 'Manual Tarefa', 'info', 'bpmn-icon-manual');
    } else if (is(element, 'bpmn:ScriptTask')) {
      self.log(elementName || 'Script Tarefa', 'info', 'bpmn-icon-script');
    } else if (is(element, 'bpmn:ServiceTask')) {
      self.log(elementName || 'Serviço da Tarefa', 'info', 'bpmn-icon-service');
    } else if (is(element, 'bpmn:UserTask')) {
      self.log(elementName || 'Usuario Tarefa', 'info', 'bpmn-icon-user');
  ...
  eventBus.on(CONSUME_TOKEN_EVENT, function(context) {
    ...
    if (is(element, 'bpmn:EndEvent')) {
      ...
      if (isTypedEvent(getBusinessObject(element), 'bpmn:TerminateEventDefinition')) {
        ...
        self.log(elementName || 'Fim do Evento', 'info', 'bpmn-icon-end-event-none');
      }
    }
  });
