...
SimulationState.prototype.isFinished = function(element, processInstanceId) {
  ...
  if (!hasTokens && !hasAnimations) {
    if (is(parent, 'bpmn:SubProcess')) {
      this._log.log('Sub-Processo ' + processInstanceId + ' Fim da Simulação', 'info', 'fa-check-circle');
    } else {
      this._log.log('Processo ' + processInstanceId + ' Fim da Simulação', 'success', 'fa-check-circle');
