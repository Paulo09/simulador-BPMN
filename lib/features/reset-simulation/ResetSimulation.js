...
ResetSimulation.prototype._init = function() {
  ...
  this.paletteEntry = domify('<div class="entry disabled" title="Redefinir Simulação"><i class="fa fa-refresh"></i></div>');

  domEvent.bind(this.paletteEntry, 'click', function() {
    self.resetSimulation();

    self._notifications.showNotification('Redefinir Simulação', 'info');
  });
